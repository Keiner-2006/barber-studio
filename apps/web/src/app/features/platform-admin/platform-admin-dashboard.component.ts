...
@Component({
    ...
    <div class="dashboard">
      ...
      <div class="kpi-grid">
        <div class="kpi-card">
          ...
          <div class="kpi-value">{{ store.totalTenants() ?? '—' }}</div>
          <div class="kpi-detail">{{ store.loading() ? 'Cargando...' : (store.offline() ? 'Sin conexión' : store.tenants().length + ' registros') }}</div>
        </div>
        <div class="kpi-card">
          <div class="kpi-header">
        ...
        <div class="kpi-card">
          ...
          <div class="kpi-value">{{ store.metrics().provisioningTenants }}</div>
        ...
        <div class="kpi-card">
          ...
          <div class="kpi-value">{{ store.metrics().suspendedTenants }}</div>
      ...
      <div class="card">
        <div class="card-header">
          ...
          <a routerLink="/platform-admin/tenants" class="link-button">Ver todos →</a>
        </div>
        <div class="card-body" style="padding:0">
          @if (store.recentTenants().length === 0) {
            <div class="empty-state">
              <p>Sin tenants registrados</p>
            </div>
          } @else {
            <table class="data-table">
              ...
              <tbody>
                @for (tenant of store.recentTenants(); track tenant.id; let i = $index) {
                  <tr>
                    <td>
                      <div class="tenant-name">
                        <span class="tenant-initial" [class]="i === 0 ? 'primary' : ''">{{ i + 1 }}</span>
                        <div>
                          <span class="tenant-title">{{ tenant.tradeName }}</span>
                          <span class="tenant-slug">{{ tenant.slug }}</span>
                        </div>
                      </div>
                    </td>
                    <td>{{ countryName(tenant.countryCode) }}</td>
                    <td>{{ businessTypeLabel(tenant.businessType) }}</td>
                    <td><span class="status-badge" [class]="statusChipClass(tenant.status)">{{ tenant.status }}</span></td>
                    <td class="date-cell">{{ tenant.createdAt | date:'dd MMM yyyy' }}</td>
                  </tr>
                }
              </tbody>
            </table>
  ...
  styles: [`
    ...
    .greeting-label { font-size: 13px; color: #6b7280; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.05em; }
    .greeting { font-family: 'DM Serif Display', serif; font-size: 36px; color: #2d2d2d; margin: 0; }
    .accent { color: #b87333; }
    ...
    .card-header h3 { font-family: 'DM Serif Display', serif; font-size: 16px; color: #2d2d2d; margin: 0 0 4px; }
    .card-header p { font-size: 12px; color: #6b7280; margin: 0; }
    ...
    .status-bar-label { font-size: 13px; color: #2d2d2d; font-weight: 500; }
    .status-bar-count { font-size: 12px; color: #6b7280; font-family: 'Plus Jakarta Sans', monospace; }
    ...
    .empty-state { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 200px; color: #6b7280; gap: 12px; }
    ...
    .provisioning-name { font-size: 13px; font-weight: 500; color: #2d2d2d; }
    .provisioning-meta { font-size: 11px; color: #6b7280; }
    ...
    .tenant-title { font-weight: 500; color: #2d2d2d; display: block; }
    .tenant-slug { font-size: 11px; color: #6b7280; font-family: monospace; }
