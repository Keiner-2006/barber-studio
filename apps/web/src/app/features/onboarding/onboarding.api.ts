...
export class OnboardingApi {
  ...
  getUploadParams(folder?: string): Observable<{ cloudName: string; uploadPreset: string; uploadUrl: string }> {
    let params = new HttpParams()
    if (folder) {
      params = params.set('folder', folder)
    }
    return this.http.get<{ cloudName: string; uploadPreset: string; uploadUrl: string }>(`${this.apiUrl}/media/upload`, { params })
  }
