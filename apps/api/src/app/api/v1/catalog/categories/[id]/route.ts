...
import { updateServiceCategorySchema } from '@/modules/catalog/presentation/schemas/catalog.schema'
...
export async function PATCH(
  ...
  try {
    const result = await withTenantRequest(request.headers, async () => {
      ...
      const category = await catalogRepository.updateCategory(id, parsed.data)
      if (!category) {
        return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    const result = await withTenantRequest(request.headers, async () => {
      const { id } = await params
      const category = await catalogRepository.deleteCategory(id)
      if (!category) {
        return NextResponse.json(
          { error: { code: 'NOT_FOUND', message: 'Categoría no encontrada' }, requestId },
          { status: 404 }
